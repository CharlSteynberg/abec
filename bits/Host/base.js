...
// ----------------------------------------------------------------------------------------------------------------------------
    if(SERVERSIDE)
    {
        ...
        extend(Host)
        ({
            mule:require("child_process"),
            able:function able(nme)
            {
                let rsl = Host.execSync(`which ${nme}`);
                return (isin(rsl,`no ${nme} in`)?FALS:TRUE);
            },
        });

        extend(MAIN)
        ({
            exec:function exec(cmd,opt,shh)
            {
                if(!isKnob(opt)){opt={}}; if(!opt.encoding){opt.encoding="utf8"};
                return enthen((fnc)=>{Host.mule(cmd,opt,(hlt,rsl,err)=>
                {
            ...
            execSync:function execSync(cmd,opt)
            {
                if(!isKnob(opt)){opt={}}; if(!opt.encoding){opt.encoding="utf8"};
                return Host.mule.execSync(cmd,opt);
            },
        });
    };
// ----------------------------------------------------------------------------------------------------------------------------
    ...
    if(SERVERSIDE)
    {
        ...
        emit("almostReady").then(function almostReady()
        {
            ...
            lookup().then(function prep(inf,err)
            {
                ...
                after(10)(function allReady()
                {
                    if(conf("Host/eventLog")){dump("all ready\n")};
                    emit("allReady",inf);
                });
            });
        });
